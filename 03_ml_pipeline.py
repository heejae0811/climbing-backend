...
# Stage 1: Correlation filtering (|r| > 0.90)
# Stage 2: RFECV + RandomForest (F1 기준)
# =====================================================
def feature_selection_hybrid(X_train, y_train, groups_train, feature_cols,
                              min_features=5, verbose=True):
    if verbose:
        print("\n[3단계] Feature Selection")
        print("  [Stage 1] Correlation filtering (threshold=0.90)")
    ...
    rfecv = RFECV(
        estimator=estimator, step=1, cv=cv, scoring="f1",
        min_features_to_select=min_features, n_jobs=-1
    )
    rfecv.fit(X_train[stage1_features], y_train, groups=groups_train)

    final_features = [f for f, s in zip(stage1_features, rfecv.support_) if s]
...
def save_fs_comparison(results_with_fs, results_without_fs,
    ...
    with pd.ExcelWriter(excel_path, engine="openpyxl") as writer:
        df_w_out    = df_with.drop(columns=["Feature_Selection"]).sort_values("F1", ascending=False)
        df_wo_out   = df_without.drop(columns=["Feature_Selection"]).sort_values("F1", ascending=False)
        df_comb_out = df_combined.sort_values(["Model", "Feature_Selection"])

        df_w_out.to_excel(writer,    sheet_name="With_FeatureSelection",    index=False)
        df_wo_out.to_excel(writer,   sheet_name="Without_FeatureSelection", index=False)
        df_comb_out.to_excel(writer, sheet_name="Combined",                 index=False)
    ...
    # ── Accuracy 꺾은선 그래프 (With vs Without Feature Selection) ──
    ...
    ax.plot(model_names, vals_w,  marker="o", linewidth=2, color="#2196F3",
            label="With Feature Selection")
    ax.plot(model_names, vals_wo, marker="o", linewidth=2, color="#FF5722",
    ...
    plt.tight_layout()
    fig.savefig(os.path.join(RESULT_DIR, "fs_comparison_accuracy_line.png"),
...
def sample_size_analysis(X, y, groups, feature_cols,
    ...
    """
    각 샘플 비율에서 With FS / Without FS 두 조건 모두 평가.
    """
    print("\n" + "=" * 70)
    print("  [Sample Size Analysis]  With FS vs Without FS")
    print("=" * 70)
    ...
    for frac in fractions:
        ...
        for rep in range(n_repeats):
            ...
            # feature selection
            try:
                sel_feats = feature_selection_hybrid(
                    X_tr, y_tr, g_tr, feature_cols,
            ...
            for model_name in model_names:
                try:
                    ...
                    all_rows.append({
                        ...
                        "feature_selection":   "With Feature Selection",
                        "repeat":              rep + 1,
...
def _plot_sample_size(df_summary, model_names, fractions):
    ...
    df_fs = df_summary[df_summary["feature_selection"] == "With Feature Selection"]
    ...
    ax.set_ylabel("Accuracy", fontsize=12)
    ax.set_title("Model Accuracy across Sample Sizes",
                 fontsize=13, fontweight="bold")
...
# =====================================================
# ③ 최종 모델 부가 결과 (Confusion Matrix, Feature Importance)
# =====================================================
...
def main():
    print("\n" + "=" * 70)
    print("  머신러닝 파이프라인: FS 비교 + Sample Size Analysis")
    print("=" * 70)
    ...
    # ── 4. 전체 데이터 With FS / Without FS 평가 ──
    models = build_models()
    ...
    print("\n" + "=" * 70)
    print("  [전체 데이터 — With Feature Selection]")
    print("=" * 70)
    ...
    # ── 6. Sample Size Analysis (With FS + Without FS) ──
    ...
    if best_model is not None:
        print(f"\n{'='*70}")
        print(f"  Best Model (With FS, F1 기준): {best_model_name}  F1={best_f1_raw:.2f}")
        print(f"{'='*70}")
