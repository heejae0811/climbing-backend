...
def save_results(results_list, y_test,
                 ...
                 X_test, feature_cols, class_names):
    print("\n[5단계] 결과 저장 및 시각화")
...
# 6단계: XAI (LIME)
# =====================================================
def xai_lime(best_model, X_train, X_test, y_test, feature_cols, class_names):
    print("\n[6단계] XAI (LIME)")
...
# 7단계: 모델 저장
# =====================================================
def save_model(best_model, feature_cols):
    print("\n[7단계] 모델 저장")
    joblib.dump(best_model,   os.path.join(RESULT_DIR, "best_model.pkl"))
    joblib.dump(feature_cols, os.path.join(RESULT_DIR, "best_features.pkl"))
...
def main():
    ...
    # 4. 학습 및 평가
    print("\n[4단계] 모델 학습 및 평가")
    print(f"  사용 Feature: 전체 {len(feature_cols)}개")
    ...
    for model_name, pipeline in models.items():
        ...
        b_model, b_proba, b_metrics = evaluate_base(
            pipeline, model_name, X_train, y_train, X_test, y_test
        )
    ...
    # 6. XAI
    xai_lime(best_model, X_train, X_test, y_test, feature_cols, class_names)
